...
/**
 * 混合：两路分数各自归一化后加权。
 * 归一化是必须的 —— 余弦相似度普遍在 0.5~0.9 之间，而关键词重叠率经常是 0，
 * 不归一化直接相加，向量分会淹没关键词分。
 */
function fuse(kw, vec, alpha = 0.5) {
  const norm = (rows) => {
    const max = Math.max(...rows.map((r) => r.score), 1e-9);
    return rows.map((r) => ({ ...r, score: r.score / max }));
  };
  const k = norm(kw), v = norm(vec);
  return k.map((r, i) => ({ item: r.item, score: alpha * r.score + (1 - alpha) * v[i].score }));
}
