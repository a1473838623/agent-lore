...
module.exports = {
  ...
  TUNING: {
    ...
    windowMinutes: 30,
    // 单次最多注入几条。按**条数**而非 token 预算裁剪：
    // 按累计 token 截断的话，一条真正相关的规则可能仅仅因为前面几条啰嗦就被丢掉，
    // 取舍依据和相关性无关；而且 300 这种数字对着 20 万上下文窗口也没有实际依据。
    injectMaxItems: 5,
    // 单条规则的长度上限。这是异常数据保护，不是预算——
    // 规则本该是一句话，几千字的多半是归因时把整段 diff 写进去了，注入没有意义。
    injectMaxRuleChars: 300,
